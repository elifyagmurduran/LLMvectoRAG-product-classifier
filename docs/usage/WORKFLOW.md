...
main.py
 ...
 ├── AzureOpenAILLMProvider(...)       ← initialized with rate limiter (RPM/TPM)
...
The rate limiter's `acquire()` method runs before the API call, sleeping if needed to stay within RPM/TPM budgets. Then calls `llm_provider.chat(system, user, response_format={"type": "json_object"})`. After the response, `record_usage(total_tokens)` updates the token budget. The LLM returns a JSON array:
...
All Azure OpenAI calls use exponential backoff (configured in `system.retry`). Retry attempts log at WARNING with wait times. This is expected behaviour — not an error. The proactive rate limiter (`llm.rate_limit`) prevents most 429 errors by pacing requests. If retries still occur, consider lowering `rpm_limit` or `tpm_limit` to match your deployment quota more closely.
