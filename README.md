...
A YAML-configurable RAG pipeline that builds a FAISS vector index from the GS1 GPC taxonomy, embeds product rows from a database, and classifies them using vector similarity + LLM inference. Includes proactive rate limiting, configurable retry strategies, and batch-level error handling for reliable long-running classification jobs.
...
Copy `.env.example` to `.env` and fill in your API keys and database credentials. All tunables (batch sizes, thresholds, column names, rate limits) live in `config.yaml`.
